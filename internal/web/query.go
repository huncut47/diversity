...
func (app *App) getPublicMessages(limit int) ([]models.Message, error) {
	...
	result := app.DB.
		Select("message.*, users.username, users.email").
		Joins("JOIN users ON users.user_id = message.author_id").
...
func (app *App) getTimelineMessages(userID int, limit int) ([]models.Message, error) {
	...
	result := app.DB.
		Select("message.*, users.username, users.email").
		Joins("JOIN users ON users.user_id = message.author_id").
		Where("message.flagged = 0 AND (users.user_id = ? OR users.user_id IN (?))", userID, subQuery).
		Order("message.pub_date DESC").
		Limit(limit).
		Find(&messages)
	return messages, result.Error
...
func (app *App) getUserFollowing(userID int, limit int) ([]string, error) {
	...
	result := app.DB.Model(&models.User{}).
		Joins("JOIN follower ON follower.whom_id = users.user_id").
