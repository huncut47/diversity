...
func (app *App) TimelineHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		app.Logger.Error("Failed to load timeline", "err", err)
	...
	if err != nil {
		app.Logger.Error("Failed to render timeline template", "err", err)
		InternalServerErrorsTotal.WithLabelValues("/", "GET").Inc()
...
func (app *App) UserTimelineHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		app.Logger.Error("Failed to load user for profile", "username", username, "err", err)
	...
	if page.User != nil {
		...
		if err != nil {
			app.Logger.Error("Failed to check following status", "follower_id", page.User.UserID, "followed_id", profileUser.UserID, "err", err)
			http.Error(w, "Failed to load user profile", http.StatusInternalServerError)
			InternalServerErrorsTotal.WithLabelValues("/{username}", "GET").Inc()
			return
		}
	...
	if err != nil {
		app.Logger.Error("Failed to load user timeline", "user_id", profileUser.UserID, "err", err)
	...
	err = app.Pages["timeline"].ExecuteTemplate(w, "layout", page)
	if err != nil {
		app.Logger.Error("Failed to render user timeline template", "username", username, "err", err)
		http.Error(w, err.Error(), http.StatusInternalServerError)
		InternalServerErrorsTotal.WithLabelValues("/{username}", "GET").Inc()
	}
}
...
func (app *App) LoginHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == http.MethodPost {
		...
		if err != nil {
			app.Logger.Error("Failed to look up user during login", "username", username, "err", err)
		...
		if user == nil || !utils.CheckPasswordHash(user.PwHash, password) {
			...
			if err != nil {
				app.Logger.Error("Failed to render login template", "err", err)
				http.Error(w, err.Error(), http.StatusInternalServerError)
		...
		if err != nil {
			app.Logger.Error("Failed to get session", "err", err)
...
func (app *App) RegisterHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == http.MethodPost {
		...
		if r.Header.Get("Content-Type") == "application/json" {
			...
			if err != nil {
				app.Logger.Error("Failed to decode registration request", "err", err)
			...
			if username == "" {
				...
				if err != nil {
					app.Logger.Error("Failed to encode error response", "err", err)
				}
				return
			}
			email = req.Email
			...
			if password != password2 {
				...
				if err != nil {
					app.Logger.Error("Failed to render register template", "err", err)
					http.Error(w, err.Error(), http.StatusInternalServerError)
		...
		if err != nil {
			app.Logger.Error("Failed to check existing user during registration", "username", username, "err", err)
			page.Error = "Internal server error"
		}
		if existingUser != nil {
...
func (app *App) AddMessageHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		app.Logger.Error("Failed to insert message", "user_id", user.UserID, "err", err)
...
func (app *App) FollowUserHandler(w http.ResponseWriter, r *http.Request) {
	...
	profileUser, err := app.getUserByUsername(username)
	if err != nil {
		app.Logger.Error("Failed to load user for follow", "username", username, "err", err)
		http.Error(w, "Failed to load user profile", http.StatusInternalServerError)
	...
	if err != nil {
		app.Logger.Error("Failed to follow user", "follower_id", user.UserID, "followed_id", profileUser.UserID, "err", err)
...
func (app *App) UnfollowUserHandler(w http.ResponseWriter, r *http.Request) {
	...
	profileUser, err := app.getUserByUsername(username)
	if err != nil {
		app.Logger.Error("Failed to load user for unfollow", "username", username, "err", err)
		http.Error(w, "Failed to load user profile", http.StatusInternalServerError)
	...
	if err != nil {
		app.Logger.Error("Failed to unfollow user", "follower_id", user.UserID, "followed_id", profileUser.UserID, "err", err)
...
func (app *App) FollowersHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		app.Logger.Error("Failed to get user ID", "username", username, "err", err)
	...
	if err != nil {
		app.Logger.Error("Failed to get followers", "user_id", userID, "err", err)
		http.Error(w, "Internal server error", http.StatusInternalServerError)
		InternalServerErrorsTotal.WithLabelValues("/fllws/{username}", "GET").Inc()
		return
	}
	...
	if err != nil {
		app.Logger.Error("Failed to marshal followers to JSON", "user_id", userID, "err", err)
		http.Error(w, "Internal server error", http.StatusInternalServerError)
		InternalServerErrorsTotal.WithLabelValues("/fllws/{username}", "GET").Inc()
		return
	}
...
func (app *App) FollowUserAPIHandler(w http.ResponseWriter, r *http.Request) {
	...
	defer func() {
		if err := r.Body.Close(); err != nil {
			app.Logger.Error("Failed to close request body", "err", err)
		}
	}()
	...
	err = json.NewDecoder(r.Body).Decode(&req)
	if err != nil {
		app.Logger.Error("Failed to decode follow/unfollow request", "username", username, "err", err)
		http.Error(w, "Invalid request body", http.StatusBadRequest)
	...
	if req.Follow != "" {
		...
		if err != nil {
			app.Logger.Error("Failed to get user ID for follow", "username", req.Follow, "err", err)
			http.Error(w, "Internal server error", http.StatusInternalServerError)
			InternalServerErrorsTotal.WithLabelValues("/fllws/{username}", "GET").Inc()
			return
		}
		...
		if err != nil {
			app.Logger.Error("Failed to follow user", "user_id", userID, "followed_id", followUserID, "err", err)
			http.Error(w, "Internal server error", http.StatusInternalServerError)
		...
		if err != nil {
			app.Logger.Error("Failed to get user ID for unfollow", "username", req.Unfollow, "err", err)
			http.Error(w, "Internal server error", http.StatusInternalServerError)
			InternalServerErrorsTotal.WithLabelValues("/fllws/{username}", "GET").Inc()
			return
		}
		...
		if err != nil {
			app.Logger.Error("Failed to unfollow user", "user_id", userID, "unfollowed_id", unfollowUserID, "err", err)
			http.Error(w, "Internal server error", http.StatusInternalServerError)
			InternalServerErrorsTotal.WithLabelValues("/fllws/{username}", "POST").Inc()
			return
		}
...
func (app *App) LatestOperationHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		app.Logger.Error("Failed to encode latest response", "err", err)
	}
}
...
func (app *App) PostUserMessageHandler(w http.ResponseWriter, r *http.Request) {
	...
	user, err := app.getUserByUsername(username)
	if err != nil {
		app.Logger.Error("Failed to check existing user", "username", req.Username, "err", err)
		http.Error(w, "Internal server error", http.StatusInternalServerError)
	...
	if err != nil {
		app.Logger.Error("Failed to add message to database", "err", err)
		http.Error(w, "Internal server error", http.StatusInternalServerError)
		InternalServerErrorsTotal.WithLabelValues("/msgs/{username}", "GET").Inc()
		return
	}
